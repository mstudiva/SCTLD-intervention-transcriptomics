...
gene2kog=read.table("Durusdinium_iso2kogClass.tab",sep="\t", fill=T)
head(gene2kog)
...
pdf(file="KOG_transmission_ofav_sym_lpv.pdf", width=7, height=8)
...
# Making a heatmap with hierarchical clustering trees: 
pdf(file="KOG_transmission_ofav_sym_fc.pdf", width=7, height=8)
pheatmap(as.matrix(ktable),clustering_distance_cols="correlation",color=color, cellwidth=15, cellheight=15, border_color="white") 
while (!is.null(dev.list()))  dev.off()